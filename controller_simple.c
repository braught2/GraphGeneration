// #include <stdio.h>
enum modes {Straight,Switch_left,Switch_right,Stop};    // tmp
...
State P(State s) {
    ...
    // Check if I am currently running straight
        // Check if there's any vehicle in front of me
            // Check if I have left lane
                // Check if there's any vehicle on my left
                    // Switch left
                // Slow down
            // Slow down
    if(vehicle_state==Straight){
        for(int i=0;i<other_vehicle.size();i++){
            if(vehicle_state)
        }
    }

    // Check if I am currently switching left
    // Check if 
